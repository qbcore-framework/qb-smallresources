...
RegisterNetEvent("QBCore:Client:EnteredVehicle", function()
	...
	while IsPedInAnyVehicle(ped, false) do
		...
		if currVehicle and currVehicle ~= false and currVehicle ~= 0 then
			...
			if
				GetVehicleHandbrake(currVehicle)
				or (GetVehicleSteeringAngle(currVehicle)) > 25.0
				or (GetVehicleSteeringAngle(currVehicle)) < -25.0
			then
				if handbrake == 0 then
					handbrake = 100
	...
	if inVeh and class ~= 8 and class ~= 13 and class ~= 14 then
		if not harnessOn then
			...
			QBCore.Functions.Progressbar(
				"harness_equip",
				Lang:t("seatbelt.remove_harness_progress"),
				5000,
				false,
				true,
				{
...
RegisterCommand("toggleseatbelt", function()
	if not IsPedInAnyVehicle(PlayerPedId(), false) or IsPauseMenuActive() then
		return
	end
